...
@section('title', 'Chỉnh sửa thông tin chất liệu')
@section('content')
    ...
    <div class="row">
        <div class="col-4">
            <div class="card">   
                ...
                <div class="card-body">
                    <form action="{{route('update_material', $material->id)}}" method="post">
                        @csrf
                        <div class="form-group">
                            <label for="name">Tên chất liệu</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{$material->name}}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="" name="industry_id">
                                <option value="{{ $material->industry_id}}">{{$material->industry->name}}</option>
                                @foreach ($list_industry as $item)
                                    @if ($item->id != $material->industry_id)
